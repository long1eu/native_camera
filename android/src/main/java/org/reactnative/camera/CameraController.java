...
public class CameraController extends CameraImplementation implements ActivityAware, LifecycleObserver {
  private Context mContext;
  private Queue<MethodChannel.Result> mPictureTakenPromises = new ConcurrentLinkedQueue<>();
  private Map<MethodChannel.Result, TakePictureRequest> mPictureTakenRequest = new ConcurrentHashMap<>();
  ...
  public CameraController(Context context, DartMessenger dartMessenger, SurfaceTexture surfaceTexture) {
    ...
    addCallback(new Callback() {
      ...
      public void onPictureTaken(CameraImplementation cameraController, final byte[] data, CameraState.Orientation deviceOrientation) {
        ...
        TakePictureRequest request = mPictureTakenRequest.remove(promise);
        final File cacheDirectory = mPictureTakenDirectories.remove(promise);
        new ResolveTakenPictureAsyncTask(data, promise, request, cacheDirectory, deviceOrientation)
  ...
  public void takePicture(final TakePictureRequest options, final MethodChannel.Result promise, final File cacheDirectory) {
    mBgHandler.post(() -> {
      mPictureTakenPromises.add(promise);
      mPictureTakenRequest.put(promise, options);
      ...
      try {
        ...
        mPictureTakenRequest.remove(promise);

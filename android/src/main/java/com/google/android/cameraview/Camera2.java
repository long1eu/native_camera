...
class Camera2 extends CameraViewImpl implements MediaRecorder.OnInfoListener, MediaRecorder.OnErrorListener {
      ...
      try (Image image = reader.acquireNextImage()) {
        ...
        if (planes.length > 0) {
          ...
          if (image.getFormat() == ImageFormat.JPEG) {
            ...
            mCallback.onPictureTaken(data, CameraState.Orientation.ORIENTATION_AUTO);
          } else {
            mCallback.onFramePreview(data, image.getWidth(), image.getHeight(), displayOrientationToOrientationEnum(mDisplayOrientation));
          }
          image.close();
        }
      }
  ...
  private CameraState.Orientation mCameraOrientation;
  ...
  void stop() {
    ...
    if (mMediaRecorder != null) {
      ...
      if (mIsRecording) {
        ...
        mCallback.onVideoRecorded(mVideoPath, CameraState.Orientation.ORIENTATION_AUTO, CameraState.Orientation.ORIENTATION_AUTO);
        mIsRecording = false;
      }
    }
  }
  ...
  void setFlash(CameraState.Flash flash) {
    ...
    CameraState.Flash saved = mFlash;
    mFlash = flash;
    if (mPreviewRequestBuilder != null) {
      updateFlash();
      if (mCaptureSession != null) {
  ...
  boolean record(String path, int maxDuration, int maxFileSize, boolean recordAudio, CamcorderProfile profile, CameraState.Orientation orientation, int fps) {
    if (!mIsRecording) {
      ...
      try {
        ...
        mCallback.onRecordingStart(mVideoPath, CameraState.Orientation.ORIENTATION_AUTO, CameraState.Orientation.ORIENTATION_AUTO);
  ...
  public void setWhiteBalance(CameraState.WhiteBalance whiteBalance) {
    ...
    CameraState.WhiteBalance saved = mWhiteBalance;
  ...
  /**
   * <p>This rewrites {@link #mCameraId} and {@link #mCameraCharacteristics}.</p>
   */
  ...
  private void collectCameraInfo() {
    ...
    mCameraOrientation = displayOrientationToOrientationEnum(mCameraCharacteristics.get(CameraCharacteristics.SENSOR_ORIENTATION));
  }
  ...
  private void stopMediaRecorder() {
    ...
    if (mVideoPath == null || !new File(mVideoPath).exists()) {
      // @TODO: implement videoOrientation and deviceOrientation calculation
      mCallback.onVideoRecorded(null, CameraState.Orientation.ORIENTATION_AUTO, CameraState.Orientation.ORIENTATION_AUTO);
      return;
    }
    // @TODO: implement videoOrientation and deviceOrientation calculation
    mCallback.onVideoRecorded(mVideoPath, CameraState.Orientation.ORIENTATION_AUTO, CameraState.Orientation.ORIENTATION_AUTO);
